@extends('layouts.app')
@section('title', 'Monthly Bills - Category E')
@section('page-title', 'Monthly Bill Management - Category E (972 Sq Ft)')
...
<div class="row g-3 mb-4">
    <div class="col-lg-3 col-md-6">
        <div class="kpi-card">
            ...
            <form class="d-flex mt-2" method="GET" action="{{ route('monthly-bills-e.index') }}">
...
<div class="chart-card">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h6 class="section-title mb-0"><i class="bi bi-list-columns-reverse me-2"></i>Bills for Category E — {{ \Carbon\Carbon::parse($selectedMonth)->format('F Y') }}</h6>
        <div>
            @if($billCount === 0)
                <form action="{{ route('monthly-bills-e.generate') }}" method="POST" class="d-inline">
                    @csrf
                    <input type="hidden" name="month" value="{{ $selectedMonth }}">
                    <button type="submit" class="btn btn-primary btn-sm"><i class="bi bi-magic me-2"></i>Generate Category E Bills for {{ \Carbon\Carbon::parse($selectedMonth)->format('F Y') }}</button>
                </form>
            @endif
        </div>
    </div>
            ...
            <tbody>
                @forelse($bills as $bill)
                    ...
                    <div class="modal fade" id="payModal{{ $bill->id }}" tabindex="-1">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <form action="{{ route('monthly-bills-e.pay', $bill->id) }}" method="POST">
                                    @csrf
                    ...
                    <div class="modal fade" id="settleModal{{ $bill->id }}" tabindex="-1">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <form action="{{ route('monthly-bills-e.settle', $bill->id) }}" method="POST">
                                    @csrf
                                    <div class="modal-header">
                ...
                @empty
                    <tr>
                        <td colspan="9" class="text-center py-4 text-muted">
                            ...
                            No Category E bills generated for {{ \Carbon\Carbon::parse($selectedMonth)->format('F Y') }} yet.
                        </td>
                    </tr>
                @endforelse
            </tbody>
...
<script>
    function checkPsid(billId) {
        ...
        fetch(`/monthly-bills/category-e/${billId}/check-psid`)
            .then(res => res.json())
            .then(data => {
